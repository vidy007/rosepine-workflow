...
tile(Monitor *m)
{
	unsigned int i, n, r, h, oe = enablegaps, ie = enablegaps, mw, my, ty, bw;
	Client *c;
	...
	for (i = 0, my = ty = m->gappoh*oe, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			// resize(c, m->wx + m->gappov*oe, m->wy + my, mw - (2*c->bw) - m->gappiv*ie, h - (2*c->bw), 0);
				resize(c, m->wx + m->gappov*oe - c->bw, m->wy, mw - 2*bw - m->gappiv*ie, h - 2*bw, bw, False);
			else
				resize(c, m->wx + m->gappov*oe - c->bw, m->wy + my, mw - 2*bw - m->gappiv*ie - c->bw, h - 2*bw - c->bw, bw, False);
			if (my + HEIGHT(c) < m->wh)
			...
			resize(c, m->wx + mw + m->gappov*oe - c->bw, m->wy + ty, m->ww - mw - 2*bw - 2*m->gappov*oe, h - 2*bw - c->bw, bw, False);
			if (ty + HEIGHT(c) < m->wh)
				ty += HEIGHT(c) + m->gappih*ie - c->bw;
		}
